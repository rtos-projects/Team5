    ...
    int domid)
{
    ...
    {
        printf("%-33s %4d %4d %9"PRIu64" %9"PRIu64" %9"PRIu64" D: %9"PRIu64" Crit:%4d Of_fl: %4d\n",
            domname,
            domid,
...
int main_sched_rt(int argc, char **argv)
{
    ...
    SWITCH_FOREACH_OPT(opt, "d:p:b:B:D:v:c:l:f:h", opts, "sched-rt", 0) {
    case 'd':
        dom = optarg;
        break;
    case 'p':
    ...
    case 'l':
        ...
        break;
    case 'f':
        offl_flag = strtol(optarg, NULL, 10);
        opt_f = 1;
        break;
 
    }

    if (cpupool && (dom || opt_p || opt_bl|| opt_bh || opt_dl || opt_v || opt_f|| opt_l)) {
        fprintf(stderr, "Specifying a cpupool is not allowed with other options.\n");
        return 1;
    }
    if (!dom && (opt_p || opt_bl || opt_bh || opt_dl|| opt_v || opt_l || opt_f)) {
        fprintf(stderr, "Must specify a domain.\n");
        return 1;
    }
    if ( (opt_v || opt_p || opt_bl|| opt_dl || opt_f) && (opt_p + opt_f+ opt_dl+ opt_bl + opt_v + opt_l != 6) ) {
        fprintf(stderr, "Must specify vcpu, period, budget, criticality\n");
        return 1;
    }
